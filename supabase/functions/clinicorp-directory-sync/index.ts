...
type StructureSummary = { processedCount:number; createdPlans:number; updatedPlans:number; createdInstallments:number; updatedInstallments:number; completedSchedules:number; reviewCount:number; invalidCount:number };
...
function confirmed(rows:Row[]){const m=new Map<string,Row>();for(const r of rows){const id=rowId(r);if(id&&String(r.PatientId??"").trim()&&String(r.PaymentConfirmed??"").toUpperCase()==="X"&&String(r.ConfirmedDate??"").trim()&&methodOk(r))m.set(id,r)}return[...m.values()]}
...
const emptyStructure=():StructureSummary=>({processedCount:0,createdPlans:0,updatedPlans:0,createdInstallments:0,updatedInstallments:0,completedSchedules:0,reviewCount:0,invalidCount:0});
...
function addStructure(a:StructureSummary,b:StructureSummary){for(const k of Object.keys(a) as (keyof StructureSummary)[])a[k]+=b[k]}
...
async function syncStructure(admin:ReturnType<typeof createClient>,unitId:number,rows:Row[],schedule:boolean){const total=emptyStructure();for(const batch of schedule?scheduleBatches(rows):chunks(rows,RPC_BATCH)){const {data,error}=await admin.rpc("upsert_clinicorp_financial_structure",{p_unit_id:unitId,p_rows:batch,p_schedule_observation:schedule,p_sync_run_id:null});if(error)throw new Error(`A estrutura financeira não pôde ser atualizada: ${error.message}`);const r=data?.[0]??{};addStructure(total,{processedCount:Number(r.processed_count??0),createdPlans:Number(r.created_plans??0),updatedPlans:Number(r.updated_plans??0),createdInstallments:Number(r.created_installments??0),updatedInstallments:Number(r.updated_installments??0),completedSchedules:Number(r.completed_schedules??0),reviewCount:Number(r.review_count??0),invalidCount:Number(r.invalid_count??0)})}return total}
async function syncPayments(admin:ReturnType<typeof createClient>,unitId:number,rows:Row[]){const total=emptyPayments();for(const batch of chunks(confirmed(rows),RPC_BATCH)){const {data,error}=await admin.rpc("apply_clinicorp_confirmed_payments",{p_unit_id:unitId,p_rows:batch,p_sync_run_id:null});if(error)throw new Error(`As baixas confirmadas não puderam ser aplicadas: ${error.message}`);const r=data?.[0]??{};addPayments(total,{processedCount:Number(r.processed_count??0),createdCount:Number(r.created_count??0),updatedCount:Number(r.updated_count??0),skippedCount:Number(r.skipped_count??0),failedCount:Number(r.failed_count??0),paidInstallments:Number(r.paid_installments??0)})}return total}
...
Deno.serve(async req=>{
  ...
  const syncUnit=async(unitCode:UnitCode)=>{let runId:number|null=null,connectionId:number|null=null;try{
    ...
    const directory=emptyDirectory(),structure=emptyStructure(),payments=emptyPayments();
    ...
    addStructure(structure,await syncStructure(admin,Number(unit.id),posted,true));
    addStructure(structure,await syncStructure(admin,Number(unit.id),received,false));
    ...
    if(!completed&&cursor>=HISTORY_FLOOR){const daysBack=unitCode==="salto_de_pirapora"?7:14;const windowTo=cursor,windowFrom=maxDate(HISTORY_FLOOR,addDays(windowTo,-daysBack));const history=rowsOf(await fetchPayments(subscriber,credentials,windowFrom,windowTo,"postDate"));addDirectory(directory,await syncDirectory(admin,Number(unit.id),history));addStructure(structure,await syncStructure(admin,Number(unit.id),history,true));addPayments(payments,await syncPayments(admin,Number(unit.id),history));windows.push({from:windowFrom,to:windowTo,rows:history.length});cursor=addDays(windowFrom,-1);completed=cursor<HISTORY_FLOOR}

    const finishedAt=new Date().toISOString(),result={directory,structure,payments};const nextConfig:Row={...config,directory_source:"payment/list",directory_last_sync_at:finishedAt,directory_recent_from:recentFrom,directory_recent_to:today,directory_backfill_floor:HISTORY_FLOOR,directory_backfill_before:cursor,directory_backfill_last_windows:windows,directory_backfill_completed_at:completed?(config.directory_backfill_completed_at??finishedAt):null,directory_last_summary:directory,financial_structure_last_summary:structure,financial_structure_last_sync_at:finishedAt};
    await Promise.all([admin.from("sync_runs").update({status:payments.failedCount||structure.invalidCount?"partial":"completed",processed_count:directory.processedCount,created_count:directory.createdPatients+structure.createdPlans+structure.createdInstallments+payments.createdCount,updated_count:directory.updatedPatients+directory.linkedPatients+structure.updatedPlans+structure.updatedInstallments+payments.updatedCount,skipped_count:directory.reviewCount+directory.invalidCount+structure.reviewCount+structure.invalidCount+payments.skippedCount,error_count:payments.failedCount+structure.invalidCount,metadata:{mode:"clinicorp_canonical_financial_structure",recent_from:recentFrom,recent_to:today,historical_windows:windows,backfill_before:cursor,backfill_completed:completed,result},completed_at:finishedAt}).eq("id",run.id),admin.from("integration_connections").update({non_secret_config:nextConfig,last_error:null}).eq("id",connection.id)]);
