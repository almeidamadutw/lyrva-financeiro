...
const allowedViewsFor = (user: UserAccount) => {
  const financialViews: View[] = ["dashboard", "journey", "reminders", "invoices", "collections", "patients"];
      ...
      <SidebarInset>
        ...
        <main className="lyvra-page min-h-[calc(100svh-4.5rem)] bg-[#f7f8f4]">
          <div className="lyvra-page-inner">
            ...
            {view === "patients" && allowedViews.has("patients") && <PatientsView unit={unit} patients={patients} loading={loadingPatients} canEdit={currentUser.operationalArea !== "support"} />}
...
function InvoicesView({ unit, obligations, onIssued }: { unit: string; obligations: InvoiceObligation[]; onIssued: (id: number) => void | Promise<void> }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-4 md:grid-cols-2"><RuleCard title="Clinicorp" label="Fonte oficial" description="Paciente, unidade, contratos, parcelas, pagamentos e dados fiscais são sincronizados diretamente do Clinicorp." /><RuleCard title="LYVRA" label="Camada operacional" description="O LYVRA usa essa base para organizar cobrança, lembretes e emissão fiscal sem duplicar a origem dos dados." /></div>
  </div>;
}
...
function PatientsView({ unit, patients, loading, canEdit }: { unit: string; patients: Patient[]; loading: boolean; canEdit: boolean }) {
